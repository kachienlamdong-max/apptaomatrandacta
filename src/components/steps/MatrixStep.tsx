...
import { 
  getStructurePartConfigs, 
  sanitizeMatrixForPartConfigs, 
  autoBalanceMatrixToTarget, 
  getMatrixMismatchWarnings, 
  STRUCTURE_OPTIONS_METADATA 
} from '../../utils/structurePresets';
...
export const MatrixStep: React.FC<MatrixStepProps> = ({
  ...
  // Feature: Auto balance Matrix to target question count for active parts
  const handleAutoFitMatrix = () => {
    const balanced = autoBalanceMatrixToTarget(matrix, partConfigs);
    onChangeMatrix(balanced);
    
    // Also sync specification
    const newSpecs: SpecificationItem[] = balanced.map((row, idx) => {
      ...
      return {
        ...
        learningObjectives: {
          nb: totalNb > 0 
            ? (existing?.learningObjectives?.nb || `Nhận biết và nêu được các khái niệm, định nghĩa cơ bản của ${row.unit || row.topic}.`)
            : '',
          th: totalTh > 0 
            ? (existing?.learningObjectives?.th || `Hiểu, giải thích, phân tích và thực hiện được các quy tắc về ${row.unit || row.topic}.`)
            : '',
          vd: totalVd > 0 
            ? (existing?.learningObjectives?.vd || `Vận dụng các kiến thức về ${row.unit || row.topic} để giải quyết bài tập và tình huống thực tiễn.`)
            : '',
          vdc: totalVdc > 0 
            ? (existing?.learningObjectives?.vdc || `Vận dụng kiến thức nâng cao về ${row.unit || row.topic} để giải quyết bài toán phức hợp.`)
            : '',
        },
        questionCount: {
    ...
    setSpecSyncNotification('⚡ Đã tự động khớp và phân bổ số câu mục tiêu vào Ma trận & Bảng đặc tả!');
    setTimeout(() => setSpecSyncNotification(null), 3500);
  };

  const handleSanitizeMatrix = () => {
    const cleaned = sanitizeMatrixForPartConfigs(matrix, partConfigs);
    onChangeMatrix(cleaned);
    setSpecSyncNotification('🧹 Đã dọn sạch các ô của phần bị khóa về 0!');
    setTimeout(() => setSpecSyncNotification(null), 3000);
  };

  const handleProceedNextStep = () => {
    const warnings = getMatrixMismatchWarnings(matrix, partConfigs);
  ...
  // Calculations for Part 1, 2, 3, 4 with strict respect to isP...Active
  const totalP1_nb = isP1Active ? matrix.reduce((sum, r) => sum + (r.part1_nb || 0), 0) : 0;
  const totalP1_th = isP1Active ? matrix.reduce((sum, r) => sum + (r.part1_th || 0), 0) : 0;
  const totalP1_vd = isP1Active ? matrix.reduce((sum, r) => sum + (r.part1_vd || 0), 0) : 0;
  const totalP1_vdc = isP1Active ? matrix.reduce((sum, r) => sum + (r.part1_vdc || 0), 0) : 0;

  const totalP2_nb = isP2Active ? matrix.reduce((sum, r) => sum + (r.part2_nb || 0), 0) : 0;
  const totalP2_th = isP2Active ? matrix.reduce((sum, r) => sum + (r.part2_th || 0), 0) : 0;
  const totalP2_vd = isP2Active ? matrix.reduce((sum, r) => sum + (r.part2_vd || 0), 0) : 0;
  const totalP2_vdc = isP2Active ? matrix.reduce((sum, r) => sum + (r.part2_vdc || 0), 0) : 0;

  const totalP3_nb = isP3Active ? matrix.reduce((sum, r) => sum + (r.part3_nb || 0), 0) : 0;
  const totalP3_th = isP3Active ? matrix.reduce((sum, r) => sum + (r.part3_th || 0), 0) : 0;
  const totalP3_vd = isP3Active ? matrix.reduce((sum, r) => sum + (r.part3_vd || 0), 0) : 0;
  const totalP3_vdc = isP3Active ? matrix.reduce((sum, r) => sum + (r.part3_vdc || 0), 0) : 0;

  const totalP4_nb = isP4Active ? matrix.reduce((sum, r) => sum + (r.part4_nb || 0), 0) : 0;
  const totalP4_th = isP4Active ? matrix.reduce((sum, r) => sum + (r.part4_th || 0), 0) : 0;
  const totalP4_vd = isP4Active ? matrix.reduce((sum, r) => sum + (r.part4_vd || 0), 0) : 0;
  const totalP4_vdc = isP4Active ? matrix.reduce((sum, r) => sum + (r.part4_vdc || 0), 0) : 0;
  ...
  // Target totals
  const grandTargetQuestions = (isP1Active ? p1_target : 0) +
    (isP2Active ? p2_target : 0) +
    (isP3Active ? p3_target : 0) +
    (isP4Active ? p4_target : 0);
  ...
  const grandTargetPoints = Number((
    (isP1Active ? targetPointsP1 : 0) +
    (isP2Active ? targetPointsP2 : 0) +
    (isP3Active ? targetPointsP3 : 0) +
    (isP4Active ? targetPointsP4 : 0)
  ).toFixed(2));

  // Discrepancy warnings between matrix and partConfigs
  const mismatchWarnings = getMatrixMismatchWarnings(matrix, partConfigs);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {/* CHẾ ĐỘ NHẮC NHỞ / WARNING REMINDER BANNER */}
      {mismatchWarnings.length > 0 && (
        <div className="bg-gradient-to-r from-amber-50 via-orange-50/60 to-amber-50 p-4 sm:p-5 rounded-2xl border-2 border-amber-300 shadow-sm animate-in fade-in slide-in-from-top-2">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-start gap-3.5 max-w-3xl">
              <div className="w-10 h-10 rounded-xl bg-amber-500/20 border border-amber-400 flex items-center justify-center text-amber-700 shrink-0 mt-0.5 shadow-2xs">
                <AlertTriangle className="w-5 h-5" />
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  <h4 className="text-sm font-extrabold text-amber-950 flex items-center gap-1.5">
                    <span>⚠️ Chế độ nhắc nhở: Ma trận chưa khớp hoàn toàn với Cấu hình đề thi</span>
                  </h4>
                  <span className="text-[11px] px-2.5 py-0.5 bg-amber-200/90 text-amber-900 font-extrabold rounded-full border border-amber-300">
                    {mismatchWarnings.length} điểm cần cân đối
                  </span>
                </div>
                <div className="space-y-1 text-xs text-amber-900">
                  {mismatchWarnings.map((w) => (
                    <div key={w.id} className="flex items-start gap-1.5 leading-relaxed">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={handleAutoFitMatrix}
                className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-amber-600 hover:bg-amber-700 shadow-md shadow-amber-200 rounded-xl transition-all"
                title="Tự động phân bổ số câu mục tiêu vào các chủ đề theo tỷ lệ chuẩn Bộ GD&ĐT (40% NB, 30% TH, 20% VD, 10% VDC)"
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span>⚡ Tự Động Khớp Nhanh Ma Trận</span>
              </button>
              <button
                type="button"
                onClick={handleSanitizeMatrix}
                className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-100 border border-slate-300 rounded-xl transition-colors shadow-2xs"
                title="Dọn sạch dữ liệu các phần đã khóa về 0"
              >
                <Lock className="w-3.5 h-3.5 text-slate-500" />
                <span>Dọn phần đã khóa</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {mismatchWarnings.length > 0 ? (
            <div className="flex items-center gap-1.5 text-xs text-amber-800 bg-amber-50 px-3 py-1.5 rounded-xl border border-amber-200">
              ...
              <span>Chế độ nhắc nhở: Có <strong>{mismatchWarnings.length}</strong> điểm chưa khớp giữa Ma trận và Cấu hình mục tiêu.</span>
            </div>
          ) : (
        ...
        <div className="flex items-center gap-2">
          {mismatchWarnings.length > 0 && (
            <button
              type="button"
              onClick={handleAutoFitMatrix}
              className="inline-flex items-center gap-1.5 py-2.5 px-3.5 bg-amber-100 hover:bg-amber-200 text-amber-900 rounded-xl font-bold text-xs border border-amber-300 transition-colors shadow-2xs"
              title="Tự động phân bổ số câu mục tiêu vào ma trận"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-700" />
              <span>⚡ Khớp Nhanh Ma Trận</span>
            </button>
          )}
          <button
      ...
      {/* MODAL CẢNH BÁO / NHẮC NHỞ KHI CHUYỂN BƯỚC HOẶC XUẤT ĐỀ */}
      {showMismatchModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="bg-white max-w-lg w-full rounded-2xl p-6 shadow-2xl border-2 border-amber-300 space-y-4 animate-in zoom-in-95">
            <div className="flex items-center gap-3 text-amber-700">
              ...
              <div>
                <h3 className="text-base font-extrabold text-slate-900">
                  Nhắc Nhở: Cân Đối Ma Trận Trước Khi Tạo Đề
                </h3>
                <p className="text-xs text-slate-500">
                  Hệ thống phát hiện một số phần trong ma trận chưa khớp với cấu hình đề thi mục tiêu:
                </p>
              </div>
            </div>

            <div className="bg-amber-50/70 p-3.5 rounded-xl border border-amber-200 text-xs text-amber-950 space-y-2 max-h-48 overflow-y-auto">
              {mismatchWarnings.map((w) => (
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Thầy/cô có thể chọn <strong>"Tự động khớp & Tạo Đề"</strong> để hệ thống tự động phân bổ đều số câu/điểm theo đúng chuẩn Bộ GD&ĐT, hoặc tiếp tục xem đề theo ma trận hiện tại.
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-end gap-2 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors"
              >
                Vẫn tiếp tục
              </button>
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md transition-colors"
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span>⚡ Tự động khớp & Tạo Đề</span>
              </button>
            </div>
          </div>
        </div>
